function Satellite(position, velocity, rotation, scale){
...
Satellite.prototype.draw = function() {
  ...
  if (!initArrayBuffer( SatelliteVertices, 3, gl.FLOAT, 'a_Position')) return -1;
  if (!initArrayBuffer( SatelliteColors, 3, gl.FLOAT, 'a_Color'))return -1;
  if (!initArrayBuffer( SatelliteNormals, 4, gl.FLOAT, 'a_Normal'))return -1;

  // Write the indices to the buffer object
  gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, indexBuffer);
  gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, SatelliteIndices, gl.STATIC_DRAW);
  ...
  gl.uniform1f(u_NormalDirection, normalDirection);
   ...
   // Draw the cube
  gl.drawElements(gl.TRIANGLES, SatelliteIndices.length, gl.UNSIGNED_BYTE, 0);
	
}



Satellite.prototype.setVelocity = function(x,y,z){
	this.velocity = normalizeVec([x,y,z]);
}


Satellite.prototype.setRotation = function(x,y,z){
	this.rotation.x = x;
	this.rotation.y = y;
	this.rotation.z = z;
...
Satellite.prototype.getScaledRadius = function(){
	var x = Math.max(this.scale[0], this.scale[1]);
	
	return this.radius*x*3;
